...
- 本仓库自 2026-08-17 起按 GPL-3.0-only 发布（项目所有者决定，`TODO.md` 的 GOV-003），版权署名 `Cr0ce11`，许可证全文在 `LICENSE`。未经项目所有者明确决定，不得更换许可证、删除或改写 `LICENSE`，也不得在文档中把授权范围描述得比 GPL-3.0 更宽或更窄。新增代码同样以 GPL-3.0-only 发布；不得引入与之不兼容的第三方代码，确需引入时先取得所有者决定并附第三方许可声明。
